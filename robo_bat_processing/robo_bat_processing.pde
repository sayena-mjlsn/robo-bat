...
void setup(){
  size(1200, 800);  // the size of the interface
  textSize(23);
  //Fullscreen();
  /*
  *loading assets
...
void draw(){
  ...
  Angle = 70;  
    ...
    x1 = lerp(x1, Value, 0.05);  // the easing function
...
void serialEvent (Serial _serialPort) {
  ...
  try {
    ...
    */
    ...
    if(inString != null){
      _currentDist = float(inString);// convert the received value to float
...
void Signal_1(){ 
  ...
  for ( float z = 210 ; z<= Value-150 ;){  
     if (Value >= 900){ 
        ...
        z = lerp(z,Value-150, 0.05);  // the easing function
        z+=35;
      }
      
      Scale = Scale + 0.1;  // redefine the scaling size for the reflected signals
  ...
  void Signal_2(){   
    ...
    for (float z = Value-100 ; z>= 230 ; z-=35){ 
       z = lerp(z, 210, 0.05);  // the easing function
      Scale = Scale + 0.1;
